...
@author: sean_lan
'''
from app.scense.utils.dbopera import db_package
import math
...
class BasePackage(object):
    ...
    def deleteItemInPackage(self,position,tag=1,backupstate =0):
        '''删除背包中的物品'''
        item = None
        for fromItem in self._items:
            if position == fromItem['position']:
                item = fromItem['itemComponent']
                self._items.remove(fromItem)
                break
        if not item:
            return False
        result = db_package.deleteItemInPack(self._packageType, item, tag =tag,backupstate =backupstate)
        if result:
            return True
        return False
    ...
    def putItemInstanceInPackDB(self,item,position,characterId):
        '''放置一个物品的实例到包裹中'''
        self.putItemByPosition(position, item)
        category = 1
        if item.baseInfo.getItemPageType()==4:
            category = 2
        db_package.putOneItemInPackage(characterId,self._packageType, item.baseInfo.getId(),\
                                        position, item.pack.getStack(), category)
        
    def putItemInPackDB(self,item,position,characterId):
        '''放置物品到数据库'''
        ...
        if item.baseInfo.getItemPageType()==4:
            category = 2
        result = db_package.putOneItemInPackage(characterId,self._packageType, item.baseInfo.getId(),\
                                        position, item.pack.getStack(), category)
        return result
