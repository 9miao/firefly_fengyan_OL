...
import random,math
from app.scense.core.fight.BattleStateMachine_new import BattleStateMachine
...
def DamageFormula(actor,enemy,attackType,skillFormulass,state = 1):
    ...
    defense = {1:enemy['physicalDefense'],
               2:enemy['magicDefense']}.get(attackType)
    attack = 0
    exec(skillFormulass)
    if attack<0:
        return attack
...
class Fight:
    ...
    def fixBattleSidePosition(self):
        ...
        for activeMember in self.activeSide.getMembers():#初始化主动方
            ...
            activeMember['chaPos'] = (self.ActiveSidePosition[eyeNo][0],\
                                      self.ActiveSidePosition[eyeNo][1])
                                    #初始角色的在战场中的位置
        ...
        for passiveMember in self.passiveSide.getMembers():#初始化主动方
            ...
            passiveMember['chaPos'] = (self.PassiveSidePosition[eyeNo][0],
                                       self.PassiveSidePosition[eyeNo][1])
    ...
    def goFight(self,actorId):
        ...
        bufftag = self.battleStateMachine.executeBuffEffects(actorId)#处理buff
        if self.fighters[actorId]['died']:
            return
    ...
    def doSkill(self,actor,skillId):
        '''进行技能攻击'''
        skillType = {True:0,False:1}.get(skillId==actor['ordSkill'])
        data = {}
        ...
        data['actionId'] = skillType#actor['chaProfessionType']*1000\
#        + {True:550,False:560}.get(skillType==1)#动作ID
        data['counterHitActionId'] = actor['chaProfessionType']*1000 + 570#受反击时的动作
        ...
        if data['enemyChaArr']:
            ...
            if skillExpendPower == 0:
                powerup = POWATTUP
                self.fighters[actor['chaBattleId']]['chaCurrentPower'] += POWATTUP
            #判断能量溢出
            if self.fighters[actor['chaBattleId']]['chaCurrentPower']>\
...
#            ddata = {}
#            ddata['chaId'] = -1#角色的Id
#            ddata['chaName'] = ''#攻击方得名称
#            ddata['chaLevel'] = 1
#            ddata['chaBattleId']= -1#角色战斗ID
#            ddata['chaProfessionType'] = -1
#            ddata['actionId'] = -1#动作ID
#            ddata['counterHitActionId'] = -1
#            ddata['isDeathOfCounterHit'] = -1
#            ddata['txtEffectId'] = -1#文字特效（暴击等闪避）
#            
##            ddata['skillId'] = -1#技能的ID
#            ddata['chaEffectId'] = -1#角色释放技能特效
#            ddata['chaEnemyEffectId'] = -1#角色技能承受特效
#            ddata['chaThrowEffectId'] = -1
#            ddata['chaAoeEffectId'] = -1#角色技能投射特效
#            ddata['chaBuffArr'] = []#角色身上的buff
#            ddata['chaBuffShowList'] = []#角色身上buff显示信息
#            ddata['chaPowerUp'] = 0#+20 power增加
#            ddata['chaPowerDown'] = 0#-20 power减少
#            ddata['chaCurrentPower'] = 0#当前能量
#            ddata['chaTotalHp'] = 0
#            ddata['chaTotalPower'] = 0#受击方的总能量
#            ddata['powerEffectId'] = 0#攻击方的能量特效
#            ddata['chaChangeHp'] = 0#±20 正负HP，可能有加血技能
#            ddata['chaCurrentHp'] = 0#角色的当前血量
#            ddata['chaExpendHp'] = 0#角色技能消耗的血量
#            ddata['chaStartPos'] = (0,0)#角色的起始坐标
#            ddata['chaTargetPos'] = (0,0)#角色的目标坐标
#            ddata['chaAttackType'] = 2
#            ddata['isCriticalBlow'] = False
#            ddata['enemyChaArr'] = []#所有受攻击者的信息
#            
#            dinfo = {}
#            dinfo['enemyChaId'] = actor['chaId']#受攻击者的角色的id
#            dinfo['enemyChaName'] = actor['chaName']#攻击方得名称
#            dinfo['enemychaLevel'] = actor['chaLevel']
#            dinfo['enemyProfessionType'] = actor['chaProfessionType']
#            dinfo['enemyBattleId'] = target#受攻击者的战场id
#            dinfo['enemyActionId'] = actor['chaProfessionType']*1000+580#受攻击者的死亡动作id
#            dinfo['enemyTxtEffectId'] = 0#受攻击者的文字特效
#            
#            dinfo['chaEffectId'] = 0#dbSkill.ALL_SKILL_INFO[actor['ordSkill']]['releaseEffect']
#            dinfo['chaEnemyEffectId'] = 0#dbSkill.ALL_SKILL_INFO[actor['ordSkill']]['bearEffect']
#            dinfo['chaThrowEffectId'] = 0#dbSkill.ALL_SKILL_INFO[actor['ordSkill']]['throwEffectId']
#            dinfo['chaEnemyAoeEffectId'] = 0#dbSkill.ALL_SKILL_INFO[actor['ordSkill']]['aoeEffectId']
#            dinfo['enemyBuffArr'] = []#受攻击者的buff列表
#            dinfo['enemyBuffShowList'] = []
#            dinfo['enemyPowerUp'] = 0#受攻击者的能量增量
#            dinfo['enemyCurrentPower'] = actor['chaCurrentPower']#受攻击者的当前能量
#            dinfo['enemyTotalHp'] = actor['chaTotalHp']#受击方的总血量
#            dinfo['enemyTotalPower'] = actor['chaTotalPower']#受击方的总能量
#            dinfo['enemyPowerEffectId'] = 0#攻击方的能量特效
#            dinfo['enemyChangeHp'] = 0#受攻击者丢失的血量
#            dinfo['enemyCurrentHp'] = actor['chaCurrentHp']#受攻击者的当前血量
#            dinfo['enemyCounterHit'] = 0#受攻击者是否反击 0没有 1反击
#            dinfo['enemyCounterHitActionId'] = 0#标识反击的动作
#            dinfo['enemyStartPos'] = actor['chaPos']#受攻击者的起始坐标
#            dinfo['enemyTargetPos'] = actor['chaPos']#受攻击者的目标坐标
#            dinfo['enemyDirection'] = actor['chaDirection']#玩家朝向右，朝向右。2--玩家朝向左
#            ddata['enemyChaArr'].append(dinfo)
#            #移除战斗成员中的死亡角色
#            if direction==1:
#                self.activeList.remove(actor['chaBattleId'])
#            else:
#                self.passiveList.remove(actor['chaBattleId'])
#            self.FightData.append(ddata)
#            self.resourceCollect(ddata)
        #
        ################################################
    ...
    def calculateDamage(self,data,actor,enemy,skillId):
        ...
        info['enemyActionId'] = enemy['chaProfessionType']*1000+570#受攻击者的动作id
        ...
        if skillDistanceType==2:#根据技能距离判断攻击者的最终位置
            if enemy['chaDirection']==1:
                data['chaTargetPos'] = [enemy['chaPos'][0]+HITDISTANCE,enemy['chaPos'][1]]
            else:
                data['chaTargetPos'] = [enemy['chaPos'][0]-HITDISTANCE,enemy['chaPos'][1]]
            #添加移动的资源
        ...
        if rate <hitRate or skillTargetType != 3:#判断是否命中，命中时的操作
            ...
            if not enemy['canBeTreat']:
                ...
                #伤害计算
                damage = DamageFormula(actor,enemy,skillAttributeType,skillFormula)
                    #获取伤害加成
