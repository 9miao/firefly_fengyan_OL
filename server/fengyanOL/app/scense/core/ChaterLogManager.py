...
@author: SIOP_09
'''
from app.scense.core.chat.ChatLog import ChatLog
...
#            pushObjectNetInterface.pushServerSendChatInfo(id, context)
    ...
    def getLog(self,id,tid):
        ...
        data={}
        players=PlayersManager().getPlayerByID(tid)
        player1=PlayerCharacter(tid)#角色实例
        data['name']=player1.baseInfo.getNickName()
        data['level']=str(player1.level._level)
        data['perfession']=player1.profession.getProfessionName()
        del player1
        if players: #如果在线的话
            if players.baseInfo.getState()>0:#如果角色在公共场景
                data['chatObjectPos']=Lg().g(316)
            else:
                data['chatObjectPos']=players.baseInfo.getSceneName()
        else: #如果角色不在线
